...
impl Validator {
	...
	fn run_rec(&mut self, fo: udf::format::FileOffset, parent: Option<&Chain<'_>>) {
		...
		for (index, table) in dataset.tables.iter().enumerate() {
			// First table entry should be names table
			if index == 0 {
				if table.type_info == udf::format::TYPE_NAMES && table.key_name == 0 {
					if table.index_name != 0 || table.related_name != 0 {
						self.errors += 1;
						eprintln!("err: names table contains invalid values");
					}
					continue;
				}
				else {
					self.warns += 1;
					eprintln!("warn: missing names table!");
				}
			}
	...
	fn validate_shape(&mut self, key_name: &udf::NameOrHash, table: &udf::format::Table) {
		...
		let success = match prim_type {
			...
			// Bit arrays are special, allow extra padding bytes
			udf::format::TYPE_PRIM_BIT => {
				((shape_len + 1) / 8) - 1 <= data_size
			},
			udf::format::TYPE_PRIM_I8 | udf::format::TYPE_PRIM_U8 => {
				shape_len == data_size
			},
			udf::format::TYPE_PRIM_I16 | udf::format::TYPE_PRIM_U16 | udf::format::TYPE_PRIM_BFLOAT16 => {
			...
			udf::format::TYPE_PRIM_DECIMAL => {
				data_size % 16 == 0 && shape_len == data_size / 16
			},
			_ => {
				self.warns += 1;
